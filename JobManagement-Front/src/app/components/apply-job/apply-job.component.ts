...
import { LucideAngularModule, Briefcase, MapPin, Calendar, DollarSign, ArrowLeft, Send, AlertCircle, CheckCircle } from 'lucide-angular';
    ...
    <div class="min-h-screen bg-slate-50">
        ...
        <div class="container mx-auto px-4 max-w-7xl">
          <div class="flex items-center justify-between h-16">
            <div class="flex items-center gap-2">
              ...
              <h1 class="text-xl font-bold text-slate-900">Apply for Job</h1>
            </div>
            <button
              (click)="goBack()"
              class="px-4 py-2 text-slate-600 hover:text-slate-900 transition-colors flex items-center gap-2">
      ...
      <div class="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div *ngIf="isLoadingJob" class="text-center py-16">
          ...
          <p class="text-slate-600 mt-4">Loading job details...</p>
        </div>

        <!-- Job Details Card -->
        <div *ngIf="!isLoadingJob && job" class="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 class="text-2xl font-bold mb-4 text-slate-900">{{ job.title }}</h2>
          ...
          <div class="mb-4">
            <h3 class="font-semibold text-slate-900 mb-2">Job Description</h3>
            <p class="text-slate-600 whitespace-pre-wrap">{{ job.description }}</p>
          </div>

          <div *ngIf="job.requirements" class="mb-4">
            <h3 class="font-semibold text-slate-900 mb-2">Requirements</h3>
            <p class="text-slate-600 whitespace-pre-wrap">{{ job.requirements }}</p>
          </div>
        </div>

        <!-- Application Form -->
        <div *ngIf="!isLoadingJob && job" class="bg-white rounded-lg shadow-md p-6">
          <h2 class="text-2xl font-bold mb-4 text-slate-900">Application Form</h2>
          ...
          <form (ngSubmit)="submitApplication()" class="space-y-6">
            ...
            <div class="flex gap-4">
              <button
                ...
                class="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium flex items-center justify-center gap-2">
        ...
        <!-- Job Not Found -->
        <div *ngIf="!isLoadingJob && !job" class="bg-white rounded-lg shadow-md p-6 text-center">
          <h2 class="text-2xl font-bold mb-4 text-slate-900">Job Not Found</h2>
          <p class="text-slate-600 mb-6">The job you're looking for doesn't exist or is no longer available.</p>
          <button
            (click)="goBack()"
